...
export const projects: Project[] = [
    ...
    },
    {
        ...
        name: "perkaholic",
        type: "Game Mod",
        description: "Hybrid DLL + Wayland overlay trainer for BO3 Zombies under Wine/Proton, bridged by a binary IPC frame at /tmp/perkaholic-ipc.bin.",
        longDescription: "An XInput proxy DLL loaded inside BO3 via WINEDLLOVERRIDES owns internal hooks, actor-table reads, refdef projection, visibility, aimbot, and ESP command production. A native Linux Wayland process creates a wlr-layer-shell surface above the fullscreen game window, consumes draw frames from the IPC file, and manages click-through input regions. Neither half crashes when the other is absent.",
        approach: "DLL built with MinGW C/C++ via make internal; overlay built as a Rust workspace with wgpu + egui rendering on a Wayland surface. IPC transport is a binary file written atomically via temp-rename on every game tick. The overlay is a pure draw-only IPC consumer — it draws nothing when frames are missing or stale and does not attach to BO3; legacy reader crates remain in the repo as reference/diagnostic code only.",
        outcome: "~4.3k LOC of C/C++ across 8 internal source files; ~18k LOC of Rust across 5 crates. Tolerates partial startup — DLL-only, overlay-only, or both — without crashing either side.",
        techStack: ["C", "Rust", "MinGW", "wgpu", "wlr-layer-shell"],
        tags: ["Game Mod", "Black Ops 3", "Reverse Engineering", "Linux", "Wayland", "Wine/Proton"],
        sourceUrl: "https://github.com/luinbytes/perkaholic",
        pageUrl: "/perkaholic",
        featured: true,
    },
    {
