...
export const projects: Project[] = [
    ...
    },
    {
        ...
        approach: "DLL built with MinGW C/C++ via make internal; overlay built as a Rust workspace with wgpu + egui rendering on a Wayland surface. IPC transport is a binary file written atomically via temp-rename on every game tick. The overlay falls back to a legacy process_vm_readv reader path when IPC frames are missing or stale, logging every state transition.",
        outcome: "~4.3k LOC of C/C++ across 8 internal source files; ~18k LOC of Rust across 6 crates. Tolerates partial startup — DLL-only, overlay-only, or both — without crashing either side.",
